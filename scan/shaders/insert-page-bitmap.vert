...
// Insert shader with bitmap-based allocation
// Allocates pages from free bitmap instead of sequential allocation
// This allows reuse of deleted pages
...
uint allocatePageFromBitmap() {
    ...
    // Distribute threads across the entire bitmap
    uint threadId = uint(gl_VertexIndex);
    uint startWord = threadId % totalWords;
    
    // Search for a free page
    // With 128M pages / 32 = 4M words, balance between coverage and GPU timeout
    // 65536 words = 2M pages coverage per thread
    uint maxSearch = min(totalWords, 65536u);
    
    for (uint i = 0; i < maxSearch; i++) {
        uint bitmapIdx = (startWord + i) % totalWords;
        ...
        if (bit < 32) {
            ...
            // Someone else got it, continue to next word
        }
    }
    
    // No free pages found in search range
