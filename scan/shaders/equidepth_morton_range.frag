...
// For round-robin binning, bins are indexed linearly (0 to totalBins-1)
// Each bin contains approximately npoints/totalBins entries (perfect equi-depth)
#version 450
...
void main() {
    // For Morton binning, bin index is computed linearly from fragment coordinates
    // The framebuffer is resolution x resolution, so we map (x, y) -> linear bin index
