// Equi-Depth Index Query - Pass 2: Edge Fragment Shader
// Like compact_edge.frag - each fragment checks ONE entry (no loops)
#version 450
...
// Binding 0: Result buffer - bitmap
...
void main() {
    ...
    // Check if within this range [st, en)
    if (binid + binrange.x < binrange.y) {
        ...
        if ((rowIdWithValid & VALID_BIT_MASK) != 0u) {
            ...
            // Check if point is in query range [x1, x2, y1, y2, z1, z2]
            if (x >= qrange[0] && x <= qrange[1] &&
                y >= qrange[2] && y <= qrange[3] &&
                z >= qrange[4] && z <= qrange[5]) {
                // Set bit in result bitmap
                uint ind = rowId >> 5;
                uint bit = 1u << (rowId & 0x1fu);
                atomicOr(result[ind], bit);
            }
        }
    }
