...
static const std::vector<std::string> distributionFilesMode65 = {
    "uniform.bin",
    "normal.bin",
    "zipf1.1.bin",
    "zipf1.3.bin",
...
enum class QueryStrategyMode65 { CENTERED, FROM_MIN };

static QueryStrategyMode65 getQueryStrategyMode65(int dataId) {
    switch (dataId) {
        case 0: return QueryStrategyMode65::CENTERED;  // Uniform
        case 1: return QueryStrategyMode65::CENTERED;  // Normal
...
// Generate 100 queries with selectivities 1%, 2%, ..., 100%
static std::vector<std::array<uint32_t, 6>> generateQueriesMode65(
    ...
    QueryStrategyMode65 strategy = getQueryStrategyMode65(dataId);
    
    std::cerr << "Query strategy: " << (strategy == QueryStrategyMode65::CENTERED ? "CENTERED" : "FROM_MIN") << "\n";
    
    for (int q = 0; q < numQueries; q++) {
        double overallSelectivity = (q + 1) * 0.01;  // 1%, 2%, ..., 100%
        ...
        for (int c = 0; c < 3; c++) {
            ...
            if (c >= ncols) {
                ...
                if (strategy == QueryStrategyMode65::CENTERED) {
...
void testBruteForceVsRasterScan100(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    if (dataId < 0 || dataId >= (int)distributionFilesMode65.size()) {
    ...
    std::string dataFile = plainDataFolder + "/" + distributionFilesMode65[dataId];
    
    std::cerr << "\n========================================\n";
    std::cerr << "MODE 65: BruteForce vs RasterScan (100 queries, 1%-100%)\n";
    std::cerr << "Distribution: " << distributionNamesMode65[dataId] << " (dataId=" << dataId << ")\n";
    ...
    const int NUM_QUERIES = 100;  // 100 queries for 1%-100% selectivity
    
    // Generate queries
    std::vector<std::array<uint32_t, 6>> queries = generateQueriesMode65(minval, maxval, ncols, dataId, NUM_QUERIES);
    ...
    if (runBruteForce) {
        ...
        for (int q = 0; q < NUM_QUERIES; q++) {
            ...
            // Print every 10th query to reduce output
            // if ((q + 1) % 10 == 0 || q == 0) {
                std::cerr << "Query " << (q + 1) << " (" << std::fixed << std::setprecision(0) << ((q + 1) * 1.0) << "%): " 
    ...
    if (runBruteForce && runRasterScan) {
        std::cerr << "\n###############################################\n";
        std::cerr << "# COMPARISON (100 queries, 1%-100% selectivity)\n";
        std::cerr << "###############################################\n";
        ...
        int mismatchCount = 0;
        for (int q = 0; q < NUM_QUERIES; q++) {
            double selectivity = (q + 1) * 1.0;  // 1%, 2%, ..., 100%
            ...
            // Print every 10th query or mismatches
            // if ((q + 1) % 10 == 0 || q == 0 || !match) {
