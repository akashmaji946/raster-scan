...
static void generateTPCCDataMode64(
...
static std::vector<std::array<uint32_t, 6>> generateTPCCQueriesMode64(
    ...
    std::cerr << "[TPC-C] Generated " << numQueries << " queries (selectivity 1%-100%)\n";
    return queries;
}
...
void testBruteForceVsRasterScanTPCC100(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    std::cerr << "\n========================================\n";
    std::cerr << "MODE 64: BruteForce vs RasterScan (TPC-C, 100 queries 1%-100%)\n";
    ...
    CPUTimer genTimer;
    genTimer.start();
    generateTPCCDataMode64(targetCustomers, data, minW, maxW, minD, maxD, minC, maxC);
    ...
    std::vector<std::array<uint32_t, 6>> queries = generateTPCCQueriesMode64(minval, maxval, ncols, NUM_QUERIES);
