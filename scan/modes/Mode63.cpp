...
static void generateTPCCDataMode63(
...
static void generateTPCCQueriesMode63(
    ...
    for (int q = 0; q < numQueries; q++) {
        double overallSelectivity = (q + 1) * (1.0 / numQueries);
    ...
    double stepPct = 100.0 / numQueries;
    std::cerr << "[TPC-C] Generated " << numQueries << " queries (selectivity " 
              << std::fixed << std::setprecision(1) << stepPct << "%-100%)\n";
}
...
void testBruteForceVsEquiDepthTPCC(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    std::cerr << "\n========================================\n";
    std::cerr << "MODE 63: BruteForce vs EquiDepth (TPC-C)\n";
    std::cerr << "Scale: " << scaleNames[scaleIdx] << " (" << targetCustomers << " customers)\n";
    std::cerr << "Queries: " << numQueries << " (selectivity step: " << std::fixed << std::setprecision(1) << selectivityStep << "%)\n";
    ...
    CPUTimer genTimer;
    genTimer.start();
    generateTPCCDataMode63(targetCustomers, data, minW, maxW, minD, maxD, minC, maxC);
    ...
    vkcore::PBuffer pointsBuffer(new vkcore::Buffer(vd));
    ...
    // Query setup
    std::vector<uint32_t> targets;
    generateTPCCQueriesMode63(minval, maxval, ncols, targets, numQueries);
    ...
    if (runBruteForce) {
        ...
        for (int i = 0; i < numQueries; i++) {
            ...
            double selectivity = (i + 1) * selectivityStep;
            std::cerr << "Query " << (i + 1) << " (" << std::fixed << std::setprecision(1) << selectivity << "%): " 
    ...
    if (runBruteForce && runEquiDepth) {
        ...
        for (int q = 0; q < numQueries; q++) {
            ...
            std::cerr << std::setw(5) << (q + 1) << " | " 
                      << std::setw(10) << std::fixed << std::setprecision(1) << selectivity << "% | "
    ...
    std::cerr << "\nMode 63 (BruteForce vs EquiDepth TPC-C) Complete.\n";
}
