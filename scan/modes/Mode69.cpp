...
static void generateTPCCDataMode69(
...
// Generate 100 random queries with 1% selectivity each
static std::vector<std::array<uint32_t, 6>> generateRandomQueriesMode69(
...
void testBruteForceVsRasterScanTPCCRandom1Pct(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    std::cerr << "\n========================================\n";
    std::cerr << "MODE 69: BruteForce vs RasterScan (TPC-C, 100 random 1% queries)\n";
    ...
    CPUTimer genTimer;
    genTimer.start();
    generateTPCCDataMode69(targetCustomers, data, minW, maxW, minD, maxD, minC, maxC);
    ...
    // Query setup - 100 random queries with 1% selectivity
    const int NUM_QUERIES = 100;
    std::vector<std::array<uint32_t, 6>> queries = generateRandomQueriesMode69(minval, maxval, ncols, NUM_QUERIES);
    ...
    if (runBruteForce && runRasterScan) {
        std::cerr << "\n###############################################\n";
        std::cerr << "# COMPARISON (100 random 1% selectivity queries)\n";
        std::cerr << "###############################################\n";
