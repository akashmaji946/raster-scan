...
static const std::vector<std::string> distributionFilesMode66 = {
    "uniform.bin",
    "normal.bin",
    "zipf1.1.bin",
    "zipf1.3.bin",
...
static const std::vector<std::string> distributionNamesMode66 = {
    "uniform",
    "normal",
    "zipf1.1",
    "zipf1.3",
...
// Generate 100 random point queries using actual data points from the dataset
static std::vector<std::array<uint32_t, 6>> generatePointQueriesMode66(
    const std::vector<uint32_t>& points,  // actual data points
    uint32_t npoints,
    int ncols,
    int numQueries = 100
) {
    ...
    for (int q = 0; q < numQueries; q++) {
        ...
        for (int c = 0; c < 3; c++) {
            ...
            if (c >= ncols) {
                ...
                // Data is column-major: points[c * npoints + pointIdx]
                uint32_t point = points[c * npoints + pointIdx];
                lo = point;
...
void testBruteForceVsRasterScanPointQueries(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    if (dataId < 0 || dataId >= (int)distributionFilesMode66.size()) {
    ...
    std::string dataFile = plainDataFolder + "/" + distributionFilesMode66[dataId];
    
    std::cerr << "\n========================================\n";
    std::cerr << "MODE 66: BruteForce vs RasterScan (100 Point Queries)\n";
    std::cerr << "Distribution: " << distributionNamesMode66[dataId] << " (dataId=" << dataId << ")\n";
    ...
    const int NUM_QUERIES = 100;  // 100 point queries
    
    // Generate random point queries using actual data points
    std::vector<std::array<uint32_t, 6>> queries = generatePointQueriesMode66(points, npoints, ncols, NUM_QUERIES);
    ...
    if (runBruteForce) {
        ...
        // Run queries
        std::cerr << "\n--- BruteForceIndex Point Query Performance (warmup + mean of " << QUERY_COUNT << " runs) ---\n";
    ...
    if (runRasterScan) {
        ...
        // Run queries
        std::cerr << "\n--- RasterScan2D Point Query Performance (warmup + mean of " << QUERY_COUNT << " runs) ---\n";
