...
static const std::vector<std::string> distributionFilesMode61 = {
    "uniform.bin",
    "normal.bin",
    "zipf1.1.bin",
    "zipf1.3.bin",
...
static const std::vector<std::string> distributionNamesMode61 = {
    "uniform",
    "normal",
    "zipf1.1",
    "zipf1.3",
...
enum class QueryStrategyMode61 { CENTERED, FROM_MIN };

static QueryStrategyMode61 getQueryStrategyMode61(int dataId) {
    switch (dataId) {
        case 0: return QueryStrategyMode61::CENTERED;  // Uniform
        case 1: return QueryStrategyMode61::CENTERED;  // Normal
        case 2: return QueryStrategyMode61::FROM_MIN;  // Zipf 1.1
        case 3: return QueryStrategyMode61::FROM_MIN;  // Zipf 1.3
        case 4: return QueryStrategyMode61::FROM_MIN;  // Zipf 1.5
        case 5: return QueryStrategyMode61::CENTERED;  // TPC-C
        default: return QueryStrategyMode61::CENTERED;
    }
}

// Generate queries with selectivities 10%, 20%, ..., 100%
static std::vector<std::array<uint32_t, 6>> generateQueriesMode61(
    ...
    QueryStrategyMode61 strategy = getQueryStrategyMode61(dataId);
    
    std::cerr << "Query strategy: " << (strategy == QueryStrategyMode61::CENTERED ? "CENTERED" : "FROM_MIN") << "\n";
    ...
    for (int q = 0; q < numQueries; q++) {
        ...
        for (int c = 0; c < 3; c++) {
            ...
            if (c >= ncols) {
                ...
                if (strategy == QueryStrategyMode61::CENTERED) {
...
void testBruteForceVsRasterScanPlain(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    if (dataId < 0 || dataId >= (int)distributionFilesMode61.size()) {
    ...
    std::string dataFile = plainDataFolder + "/" + distributionFilesMode61[dataId];
    
    std::cerr << "\n========================================\n";
    std::cerr << "MODE 61: BruteForce vs RasterScan Comparison\n";
    std::cerr << "Distribution: " << distributionNamesMode61[dataId] << " (dataId=" << dataId << ")\n";
    ...
    // Generate queries
    std::vector<std::array<uint32_t, 6>> queries = generateQueriesMode61(minval, maxval, ncols, dataId, NUM_QUERIES);
