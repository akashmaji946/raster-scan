...
void testCompactIndexBatchCycles(int dataId, vkcore::PVkDevice vd, vkcore::PBuffer staging, OperatorCache &op) {
    ...
    uint32_t batchSize = 1; // npoints / NUM_BATCHES;
    ...
    // CPU reference: track which points are valid
    std::vector<bool> cpuValidMask(npoints, true);
    ...
    for (uint32_t b = 0; b < 2; b++) {
        ...
        // Update CPU reference (mark deleted)
        for (uint32_t i = 0; i < currentBatchSize; i++) {
            uint32_t pointIdx = indices[startIdx + i];
        ...
        // Update CPU reference (mark re-inserted)
        for (uint32_t i = 0; i < currentBatchSize; i++) {
            uint32_t pointIdx = indices[startIdx + i];
    ...
    std::cerr << std::string(82, '-') << "\n";
    std::cerr << "\n--- Summary ---\n";
    std::cerr << "Batches passed: " << totalPassed << "/" << NUM_BATCHES << "\n";
    ...
    std::cerr << "Avg delete time per batch: " << std::fixed << std::setprecision(3) << (totalDelTime * 1000.0 / NUM_BATCHES) << " ms\n";
    std::cerr << "Avg insert time per batch: " << std::fixed << std::setprecision(3) << (totalInsTime * 1000.0 / NUM_BATCHES) << " ms\n";
    ...
    std::cerr << "\nMode 23 Complete. Overall: " << (totalPassed == NUM_BATCHES ? "ALL PASSED" : "SOME FAILED") << "\n";
}
